...
class Trainer:
    def __init__(self, data_loaders, dataset, parameter):
        ...
        self.gta = GTA(dataset, parameter, self.num_symbols, embed=self.symbol2emb, update_embeddings=self.update_embeddings)
        self.gta.to(self.device)
        self.optimizer = torch.optim.Adam(self.gta.parameters(), self.learning_rate)
    ...
    def load_embed(self):
        ...
        if self.embed_model in ['DistMult', 'TransE', 'ComplEx', 'RESCAL']:
            ...
            ent_update_embed = np.loadtxt(self.data_path + '/emb' + '/entity2vec_degree3.' + self.embed_model)
            ...
            i = 0
            embeddings = []
            update_ent_embeddings = []
            for key in rel2id.keys():
                if key not in ['','OOV']:
                    symbol_id[key] = i
                    i += 1
                    embeddings.append(list(rel_embed[rel2id[key],:]))
                    update_ent_embeddings.append(list(rel_embed[rel2id[key],:]))
            ...
            for key in self.ent2id.keys():
                if key not in ['', 'OOV']:
                    ...
                    update_ent_embeddings.append(list(ent_update_embed[self.ent2id[key],:]))

            symbol_id['PAD'] = i
            embeddings.append(list(np.zeros((rel_embed.shape[1],))))
            embeddings = np.array(embeddings)
            update_ent_embeddings.append(list(np.zeros((rel_embed.shape[1],))))
            update_ent_embeddings = np.array(update_ent_embeddings)
            ...
            self.update_embeddings = update_ent_embeddings
    ...
    def get_task_meta(self, task, iseval):
        ...
        for triplet_list in task:
            ...
            task_meta.append([self.get_meta(triplet_meta_left[i], triplet_meta_right[i], iseval) for i in range(len(triplet_meta_left))])
        
        return task_meta
    
    def get_meta(self, left, right, iseval):
        ...
        if not iseval:
            left_ent_degrees=[int(id) for id in list(left_degrees)]
            right_ent_degrees=[int(id) for id in list(right_degrees)]
            for idx in range(left_connections.shape[0]):
                if left_ent_degrees[idx]>5:
                    left_connections[idx,0,1]=torch.randint(0, len(self.rel2id), (1,)).to(self.device)
                    left_connections[idx,0,2]=torch.randint(len(self.rel2id), self.num_symbols, (1,)).to(self.device)
                if right_ent_degrees[idx]>5:
                    right_connections[idx,0,1]=torch.randint(0, len(self.rel2id), (1,)).to(self.device)
                    right_connections[idx,0,2]=torch.randint(len(self.rel2id), self.num_symbols, (1,)).to(self.device)
    ...
    def get_negative_meta(self, meta):
        ...
        for few_id in range(self.few):
            ...
            left_ent_degrees=[int(id) for id in list(left_degrees)]
            right_ent_degrees=[int(id) for id in list(right_degrees)]
            for idx in range(triplet_num):
                false_left_connections[idx, 0:min(left_ent_degrees[idx],neighbor_num), 1] = torch.randint(0, len(self.rel2id), (1, min(left_ent_degrees[idx],neighbor_num))).to(self.device)
                false_left_connections[idx, 0:min(left_ent_degrees[idx],neighbor_num), 2] = torch.randint(len(self.rel2id), self.num_symbols, (1, min(left_ent_degrees[idx],neighbor_num))).to(self.device)
                false_right_connections[idx, 0:min(right_ent_degrees[idx],neighbor_num), 1] = torch.randint(0, len(self.rel2id), (1, min(right_ent_degrees[idx],neighbor_num))).to(self.device)
                false_right_connections[idx, 0:min(right_ent_degrees[idx],neighbor_num), 2] = torch.randint(len(self.rel2id), self.num_symbols, (1, min(right_ent_degrees[idx],neighbor_num))).to(self.device)
    ...
    def reload(self):
        ...
        if os.path.isfile(state_dict_file):
            self.gta.load_state_dict(state)
        else:
            raise RuntimeError('No state dict in {}!'.format(state_dict_file))

    def save_checkpoint(self, epoch):
        torch.save(self.gta.state_dict(), os.path.join(self.ckpt_dir, 'state_dict_' + str(epoch) + '.ckpt'))
    ...
    def do_one_step(self, task, iseval=False, curr_rel='', istest=False):
        ...
        if not iseval:
            ...
            sup_neg_meta=None
            p_score, n_score, positive_context, negative_context, sup_neg_context, support_emb = self.gta(task, iseval, curr_rel, support_meta, support_negative_meta, sup_neg_meta)
            negative_context = torch.stack(negative_context, dim=1)
            y=torch.ones_like(p_score).to(self.device)
            loss_margin = self.gta.loss_func(p_score, n_score, y)
            ...
            p_score, n_score, _, _, _, _ = self.gta(task, iseval, curr_rel, support_meta)
            y=torch.ones_like(p_score).to(self.device)
            loss_margin = self.gta.loss_func(p_score, n_score, y)
            loss_info = 0
