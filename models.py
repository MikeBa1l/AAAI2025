...
class transformer_block(nn.Module):
    def __init__(self, dim, out_dim, num_heads, mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0.,
        ...
        self.attn2=Attention(dim//2, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=0.2, proj_drop=drop)
        self.gate_w = nn.Linear(dim//2,1)
    ...
    def forward(self, x, context):
        ...
        context=context.contiguous().view(size[0],-1,context.shape[1])  
        context,_=self.attn2(self.norm3(context))
        gate_tmp=self.gate_w(context)+self.gate_b
        gate=torch.sigmoid(gate_tmp)
        x=torch.mul(x,1.0-gate)+torch.mul(context, gate)
        return x.mean(dim=1).view(size[0], 1, 1, self.out_dim), x.view(-1, self.out_dim)

class ContextLearner(nn.Module):
    def __init__(self, num_symbols, embed, update_embed, embed_dim, few, batch_size,
        ...
        self.symbol2emb_context = nn.Embedding(num_symbols + 1, self.embed_dim, padding_idx=self.num_symbols)
        self.symbol2emb_context.weight.data.copy_(torch.from_numpy(update_embed))
        self.symbol2emb_context.weight.requires_grad = True
    ...
    def forward(self, connections, mask, is_pos_context=True):
        relations = connections[:, :, :, 0]
        entities = connections[:, :, :, 1]
        rel_embeds = self.symbol2emb(relations)
        if is_pos_context:
            entity_embeds = self.symbol2emb_context(entities)
        else:                          
...
class GTA(nn.Module):
    def __init__(self, dataset, parameter, num_symbols, embed=None, update_embeddings=None):
        ...
        if parameter['dataset'] == 'Wiki-One':
            self.context_learner = ContextLearner(num_symbols, embed, update_embeddings, self.embed_dim, self.few, self.batch_size, dim=100, num_heads=1, drop=0.2, drop_path=0.2)
            ...
            self.context_learner = ContextLearner(num_symbols, embed, update_embeddings, self.embed_dim, self.few, self.batch_size, dim=200, num_heads=1, drop=0.2, drop_path=0.2)
    ...
    def build_context(self, meta, iseval):
        ...
        if not iseval:
            ...
            left_digits = torch.zeros(self.few, self.max_neighbor).to(self.device)
            right_digits = torch.zeros(self.few, self.max_neighbor).to(self.device)
        ...
        left_connections=left_connections[:,:,:,1:]
        right_connections=right_connections[:,:,:,1:]
        ...
        if not iseval:
            ...
            return connections, mask.reshape(1, self.few, self.max_neighbor*2, self.max_neighbor*2)

    def forward(self, task, iseval=False, curr_rel='', support_meta=None, support_negative_meta=None, sup_neg_meta=None):
        ...
        if not iseval:
            positive_connections, positive_mask = self.build_context(support_meta,iseval)
            negative_connection_mask = [self.build_context(support_nn,iseval) for support_nn in support_negative_meta]
            ...
            sup_neg_context=None
        else:
            positive_connections, positive_mask = self.build_context(support_meta,iseval)
            positive_context = self.context_learner(positive_connections, positive_mask)
            negative_context, sup_neg_context = None, None

        rel, support_emb = self.relation_learner(support.contiguous().view(batch_size, few, -1), positive_context)
        ...
        return p_score, n_score, positive_context, negative_context, sup_neg_context, support_emb
